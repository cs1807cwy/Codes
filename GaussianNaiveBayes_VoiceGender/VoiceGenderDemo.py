...
def main_cross(seed=1, detail_show=False, roc_plot=False, global_plot=False):
    ...
    labels = ['(Mean)', '(Var)', '(Mean,Denoise)', '(Var,Denoise)']
    # 平均准确度制图
...
Parameters:
    ...
    weakness - 附加集成分类器强度 default:0.3
"""
def main_predict(train_csv, test_csv, features=1, extraction='var', denoise=False, bagging=0, weakness=0.3):
    ...
    fixDeafults(df_train, discard=True)
    fixDeafults(df_test, discard=False)
    train, test = generateMainFeatures(df_train, df_test, features, extraction)
    #print(train.columns.values.tolist())
    clf = GaussianNaiveBayesClassfier(has_denoise=denoise, bagging_rate=bagging, bagging_weakness=weakness)
    clf.fit(train)
    #clf.values()
    voicePredict = clf.predict(test.iloc[:, 0:-1])
    gender = ['female', 'male']
...
if __name__ == '__main__':
    #createDataset(num=100)
    main_cross(10, global_plot=True)
    #main_in(10, 20, None, False, 0)
    #main_in(10, 20, 'var', False, 0)
    #main_in(1, 20, 'var', True, 3, 0.5, detail_show=False, roc_plot=False, global_plot=True)
    #main_in(10, 10, 'var', False, 0, detail_show=True, roc_plot=True, global_plot=True)
    #main_in(1, 20, 'var', True, 0)
    #main_in(10, 20, 'var', False, 10, 0.9)
    #main_in(10, 20, 'var', True, 0)
    #main_in(10, 20, 'var', True, 3, 0.3, detail_show=False, roc_plot=False, global_plot=True)
    #main_in(10, 20, 'var', True, 3, 0.5, detail_show=False, roc_plot=False, global_plot=True)
    #main_in(10, 20, 'var', True, 3, 0.7, detail_show=False, roc_plot=False, global_plot=True)
    #main_in(10, 20, 'var', True, 3, 0.9, detail_show=False, roc_plot=False, global_plot=True)
    #main_in(10, 3, 'var', True, 3, 0.9)
    #main_predict('voice/voice_train_1.csv', 'voice/predict/predict.csv', 4, 'var', False, 0)
